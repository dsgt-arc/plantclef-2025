    ...
    image_parts = [
        types.Part.from_text(
            text=(
                ...
                "and provide the JSON output containing the results for each image:\n"
...
def validate_output(image_paths: List[Path], response: str) -> bool:
    ...
    try:
        ...
        response_names = [item.get("image_name") for item in json_data]
        return set(image_names) == set(response_names) and len(json_data) > 0
    except Exception:
        return False
...
def process_batch(
    ...
    output_file_path = output_dir_path / f"results/batch-{batch_index:03d}.json"
